            ...
            <div class="field full">
                ...
                <select name="categories[]" multiple size="6">
                    @foreach($categories as $cat)
                        <option value="{{ $cat['id'] }}" @selected(in_array($cat['id'], $v('categories', [])))>
                            {{ $cat['title'] }}
                        </option>
                    @endforeach
                </select>
                <small>First selected category becomes the primary category.</small>
            </div>
            
            <div class="field full">
                <label>Related Products</label>
                <select name="related[]" multiple size="5">
                    @foreach($products as $product)
                        <option value="{{ $product['id'] }}" @selected(in_array($product['id'], $v('related', [])))>
                            {{ $product['title'] }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="form-grid">
            <div class="field">
                ...
                <select name="parent_id">
                    ...
                    @foreach($categories as $cat)
                        @if(!$item || $cat['id'] !== $item['id'])
                            <option value="{{ $cat['id'] }}" @selected($v('parent_id') === $cat['id'])>
                                {{ $cat['title'] }}
                            </option>
                        @endif
                    @endforeach
    ...
    <div class="section">
        ...
        <div class="form-grid">
            <div class="field">
                ...
                <input name="products_heading" value="{{ $v('products_heading') }}">
            </div>
            
            <div class="field">
                <label>Feature Title</label>
                <input name="feature_title" value="{{ $v('feature_title') }}">
            </div>
            
            <div class="field full">
                <label>Products Description</label>
                <textarea name="products_description">{{ $v('products_description') }}</textarea>
            </div>
            
            <div class="field">
                <label>Why Choose Title</label>
                <input name="why_choose_title" value="{{ $v('why_choose_title') }}">
            </div>
            
            <div class="field">
                <label>Image Badge</label>
                <input name="image_badge" value="{{ $v('image_badge') }}">
            </div>
            
            <div class="field full">
                <label>Why Choose Description</label>
                <textarea name="why_choose_description">{{ $v('why_choose_description') }}</textarea>
            </div>
...
file_put_contents('resources/views/admin/forms/products.blade.php', $commonHeader . "\n" . $productsContent . "\n" . str_replace('<!-- MODULE_VISIBILITY_SLOT -->', '', $commonFooter));
file_put_contents('resources/views/admin/forms/categories.blade.php', $commonHeader . "\n" . $categoriesContent . "\n" . str_replace('<!-- MODULE_VISIBILITY_SLOT -->', '<label class="check"><input type="checkbox" name="show_in_nav" value="1" @checked($v(\'show_in_nav\'))> Show in navigation</label>', $commonFooter));
file_put_contents('resources/views/admin/forms/blogs.blade.php', $commonHeader . "\n" . $blogsContent . "\n" . str_replace('<!-- MODULE_VISIBILITY_SLOT -->', '<label class="check"><input type="checkbox" name="set_home" value="1" @checked($v(\'set_home\'))> Feature on home</label>', $commonFooter));
file_put_contents('resources/views/admin/forms/pages.blade.php', $commonHeader . "\n" . $pagesContent . "\n" . str_replace('<!-- MODULE_VISIBILITY_SLOT -->', '', $commonFooter));
echo "Created new views!\n";
